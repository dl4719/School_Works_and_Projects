...
int main(int argc, char** argv)
{
...

	{
		...
		try
		{
			server.receiveMail(messages[0]);  // Trying to receive the first message
		}
		catch (const char* e)
		{
			std::cout << "EXCEPTION: Message intended for [" << messages[0].m_toAddress << "]" << std::endl;  // Handle exception and print the error message
		}
		...
		auto fixDestAddress = [](seneca::Message& msg)
			{
				...
				{
					msg.m_toAddress = "john.smith@senemail.ca";  // Fix the address if it's empty
				}
			};
